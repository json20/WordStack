...
public class MainActivity extends AppCompatActivity {
        ...
        public boolean onDrag(View v, DragEvent event) {
            ...
            switch (event.getAction()) {
                ...
                case DragEvent.ACTION_DROP:
                    ...
                    placedTiles.push(tile);
                    //if (placedTiles.size() == (word1.length() + word2.length())) &&
                    return true;
            }
            return false;
        }
    ...
    public boolean onStartGame(View view) {
        ...
        while ((counterWord1 < word1.length()) && (counterWord2 < word2.length())) {
            ...
            if (pickWord == 0) {
                scrambledString += word1.charAt(counterWord1);
                counterWord1 += 1;
            }
            else {
                scrambledString += word2.charAt(counterWord2);
                counterWord2 += 1;
            }
        }
        if (counterWord1 < word1.length()) {
            scrambledString += word1.substring(counterWord1, word1.length());
        }
        else if (counterWord2 < word2.length()) {
            scrambledString += word2.substring(counterWord2, word2.length());
        }
        for (int i = scrambledString.length() - 1; i > - 1; i -= 1) {
            LetterTile l = new LetterTile(getApplicationContext(), scrambledString.charAt(i));
            stackedLayout.push(l);
        }
